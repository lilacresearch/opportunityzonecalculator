require 'date'
    ...
    class MarriedFilingSeparately
        ...
        def determineRate
            ...
            #Married TPs filing separately with incomes over $125,000 per year are subject to an additional 3.8% Net Investment Income Tax.
            if (@income > 125000)
            ...
            case
            when @income >= 306175  #if the taxpayer makes more than $510,300.00 then they are in the 37% tax bracket.
                @rate = 0.37
                
            when ((@income < 306175) and (@income >= 204100))
                @rate = 0.35

            when ((@income < 204100) and (@income >= 160725))
                @rate = 0.32

            when ((@income < 160725) and (@income >= 84200))
                @rate = 0.24
        
            when ((@income < 84200) and (@income >= 39475))
                @rate = 0.22

            when ((@income < 39475) and (@income >= 9700))
                @rate = 0.12

            when ((@income < 9700) and (@income >= 0))
                @rate = 0.10
